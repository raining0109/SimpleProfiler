...
static void signalHandler(int signo, siginfo_t *siginfo, void *ucontext) {
    ...
    //此时，我们应该已经获取到了堆栈trace
    fprintf(stdout, "---------------------\n");
    fprintf(stdout, "trace.num_frames: %d\n", trace.num_frames);
    for (int i = 0; i < trace.num_frames; ++i) {
        char *method_sig;
        jvmti->GetMethodName(trace.frames[i].method_id, &method_sig, NULL, NULL);
        fprintf(stdout, "frame %d -> [lineno:%d, method_id:%s]\n", i, trace.frames[i].lineno, method_sig);
    }
